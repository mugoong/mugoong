  ...
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <svg
            className={`h-7 w-7 transition-colors ${n <= (hover || value) ? 'text-yellow-400' : 'text-gray-200'}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <svg
          key={n}
          className={`h-4 w-4 ${n <= rating ? 'text-yellow-400' : 'text-gray-200'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
...
export default function ReviewsSection({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/reviews', {
      ...
      body: JSON.stringify({ listing_id: listingId, listing_title: listingTitle, rating, title, content }),
    });
    ...
    <div>
      ...
      {data === null ? null : !data.canReview && !data.myReview ? (
        ...
        <div className="mb-6 rounded-xl border border-primary-100 bg-primary-50 p-4 text-sm text-primary-700">
          ...
          <a href={`/${locale}/account`} className="underline">
        ...
        <form onSubmit={handleSubmit} className="mb-8 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div className="mb-4">
            <label className="mb-1.5 block text-sm font-medium text-gray-700">
              Review *{' '}
              <span className="text-xs font-normal text-gray-400">(min. 10 characters)</span>
            </label>
            <textarea
              value={content}
        ...
        <div className="space-y-4">
          {reviews.map((r) => (
            <div key={r.id} className="rounded-xl border border-gray-100 bg-white p-5">
              ...
              {r.title && (
                <p className="mt-3 font-semibold text-gray-800">{r.title}</p>
              )}
