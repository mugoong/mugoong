...
export default function RestaurantFormFields({ menuItems, setMenuItems, extra, setExtra, operatingHours, setOperatingHours, address, setAddress, phone, setPhone }: Props) {
  ...
  const addReview = () => {
    ...
    setExtra('external_reviews', [...reviews, { source: '', reviewer: '', rating: 5, text: '', date: '' }]);
  };
    ...
    <>
      ...
      <section className="rounded-xl border border-gray-200 bg-white p-6">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">⭐ External Reviews ({reviews.length}/10)</h2>
          {reviews.length < 10 && <button type="button" onClick={addReview} className="rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200">+ Add Review</button>}
        </div>
        ...
        {reviews.map((r, i) => (
          <div key={i} className="mb-3 rounded-lg bg-gray-50 p-4 space-y-2">
            ...
            <textarea value={r.text} onChange={e => updateReview(i, 'text', e.target.value)} placeholder="Review text..." rows={2} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none" />
