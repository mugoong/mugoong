...
function parseExtra(notes?: string): Record<string, any> {
  ...
  try {
    ...
    if (p && typeof p === 'object' && p.__extra) return p.__extra;
  } catch {}
  return {};
}
...
function ReviewCard({ review, td }: { review: any; td: (k: string) => string }) {
  ...
  return (
    <div className="rounded-xl border border-gray-100 p-4">
      ...
      {hasTr && (
        <>
          <button onClick={() => setShowTr(!showTr)} className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-blue-500 hover:text-blue-700 transition">
            🌐 {showTr ? td('showOriginal') : td('seeEnglishTranslation')}
          </button>
...
/* ── Gallery grid ── */
function GalleryGrid({ images, onImageClick }: { images: string[]; onImageClick: (i: number) => void }) {
  const [expanded, setExpanded] = useState(false);
  const INIT = 5;
  const extra = Math.max(0, images.length - INIT);
  const visible = expanded ? images : images.slice(0, INIT);

  return (
    <div>
      <div className="relative aspect-[16/9] cursor-pointer overflow-hidden rounded-2xl" onClick={() => onImageClick(0)}>
        <Image src={images[0]} alt="" fill className="object-cover" priority sizes="(max-width:1024px) 100vw,800px" />
      </div>
      {images.length > 1 && (
        <div className="mt-2 grid grid-cols-4 gap-2">
          {visible.slice(1, 5).map((img, i) => {
            const idx = i + 1;
            const isLast = i === 3 && extra > 0 && !expanded;
            return (
              <div key={idx} className="relative aspect-[4/3] cursor-pointer overflow-hidden rounded-lg"
                onClick={() => isLast ? setExpanded(true) : onImageClick(idx)}>
                <Image src={img} alt="" fill className="object-cover transition-transform duration-300 hover:scale-105" sizes="25vw" />
                {isLast && (
      ...
      {expanded && extra > 0 && (
        <div className="mt-2 grid grid-cols-4 gap-2">
          {images.slice(5).map((img, i) => (
            <div key={i+5} className="relative aspect-[4/3] cursor-pointer overflow-hidden rounded-lg" onClick={() => onImageClick(i+5)}>
              <Image src={img} alt="" fill className="object-cover transition-transform duration-300 hover:scale-105" sizes="25vw" />
            </div>
          ))}
        </div>
...
function Lightbox({ images, idx, onClose, setIdx }: { images: string[]; idx: number; onClose: () => void; setIdx: (i: number) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90" onClick={onClose}>
      <button className="absolute right-4 top-4 z-10 rounded-full bg-white/10 p-2 text-white hover:bg-white/20"
        onClick={(e) => { e.stopPropagation(); onClose(); }}>✕</button>
      <div className="relative flex w-full max-w-5xl items-center px-14"
...
function MoreDetailsModal({
  listing, category, subcategory, onClose,
}: {
  listing: Listing; category: CategoryConfig; subcategory: SubCategory; onClose: () => void;
}) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[101] flex items-end justify-center sm:items-center sm:p-4">
        <div className="relative flex w-full max-w-3xl flex-col rounded-t-3xl bg-white sm:max-h-[90vh] sm:rounded-3xl" style={{ maxHeight: '92vh' }}>

          {/* Sticky header */}
          <div className="sticky top-0 z-10 flex flex-shrink-0 items-center justify-between rounded-t-3xl border-b border-gray-100 bg-white/95 px-6 py-4 backdrop-blur sm:rounded-t-3xl">
            <h2 className="truncate pr-4 text-lg font-bold text-gray-900">{listing.title}</h2>
            <button onClick={onClose} className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition text-lg">✕</button>
          </div>
          ...
          <div className="overflow-y-auto px-6 pb-10 pt-6">
            ...
            {/* Gallery */}
            <GalleryGrid images={allImages} onImageClick={setLbIdx} />
              ...
              <>
                ...
                {(extra.map_description || extra.naver_map_url || extra.kakao_map_url || extra.google_map_url) && (
                  <div className="mt-8 rounded-xl border border-gray-100 p-5">
                    ...
                    <MapButtons />
                    {extra.google_map_url && <div className="mt-4 overflow-hidden rounded-lg"><iframe src={`https://www.google.com/maps?q=${encodeURIComponent(listing.address || extra.map_description || '')}&output=embed`} width="100%" height="250" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div>}
                  </div>
                )}
              </>
            )}
            ...
            {isRestaurant && (
              <>
                            ...
                            {items.map((item, i) => (
                              <div key={i} className="flex items-start gap-3 px-4 py-3">
                                ...
                                <div className="flex flex-1 items-center justify-between">
                                  <div><p className="font-medium text-gray-800">{item.name}</p>{item.description && <p className="text-xs text-gray-500">{item.description}</p>}</div>
                ...
                <div className="mt-8 rounded-xl border border-gray-100 p-5">
                  ...
                  <MapButtons />
                  {extra.google_map_url && <div className="mt-4 overflow-hidden rounded-lg"><iframe src={`https://www.google.com/maps?q=${encodeURIComponent(listing.address)}&output=embed&t=k`} width="100%" height="250" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div>}
                </div>
                {extra.reservation_notices?.length > 0 && (
                  <div className="mt-8"><SectionHeader icon={<InfoCircleIcon className="h-5 w-5" />} title={td('reservationInfo')} /><div className="space-y-2">{extra.reservation_notices.map((n: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg bg-blue-50 p-3"><span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-blue-200 text-xs font-bold text-blue-700">{i+1}</span><span className="text-sm text-gray-700">{n}</span></div>)}</div></div>
                )}
                {extra.cancellation_policy?.length > 0 && (
                  <div className="mt-8"><SectionHeader icon={<RefreshIcon className="h-5 w-5" />} title={td('cancellationPolicy')} /><div className="space-y-2">{extra.cancellation_policy.map((p: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg bg-gray-50 p-3"><span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-gray-400" /><span className="text-sm text-gray-700">{p}</span></div>)}</div></div>
                )}
                {extra.important_notes?.length > 0 && (
                  <div className="mt-8"><SectionHeader icon={<WarningIcon className="h-5 w-5" />} title={td('thingsToKnow')} /><div className="grid gap-2 sm:grid-cols-2">{extra.important_notes.map((n: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg border border-amber-100 bg-amber-50 p-3"><span className="mt-0.5 flex-shrink-0 text-amber-500"><WarningIcon className="h-4 w-4" /></span><span className="text-sm text-gray-700">{n}</span></div>)}</div></div>
                )}
              </>
            )}
            ...
            {cat === 'wellness' && (
              <>
                ...
                {listing.subcategory === 'sauna' && (extra.adult_price || extra.child_price) && (
                  <div className="mt-8">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      {extra.adult_price > 0 && <div className="rounded-xl border border-gray-100 p-4 text-center"><p className="text-sm font-medium text-gray-500">{td('adultAgeLabel')}</p><p className="mt-1 text-2xl font-bold text-primary-600">₩{Number(extra.adult_price).toLocaleString()}</p></div>}
                      {extra.child_price > 0 && <div className="rounded-xl border border-gray-100 p-4 text-center"><p className="text-sm font-medium text-gray-500">{td('childAgeLabel')}</p><p className="mt-1 text-2xl font-bold text-primary-600">₩{Number(extra.child_price).toLocaleString()}</p></div>}
                    </div>
                  </div>
                )}
                {listing.subcategory === 'sauna' && extra.facilities?.length > 0 && (
                  <div className="mt-8"><SectionHeader icon={<FacilitiesIcon className="h-5 w-5" />} title={td('facilitiesAndAmenities')} /><div className="grid gap-2 sm:grid-cols-2">{extra.facilities.map((f: string, i: number) => <Check key={i}>{f}</Check>)}</div></div>
                )}
                {extra.staff?.length > 0 && (
                  <div className="mt-8">
                    <SectionHeader icon={listing.subcategory === 'skin-clinic' ? <DoctorIcon className="h-5 w-5" /> : <ScissorsIcon className="h-5 w-5" />} title={listing.subcategory === 'skin-clinic' ? td('ourDoctors') : td('ourHairDesigners')} />
                    <div className="grid gap-4 sm:grid-cols-2">
                      {extra.staff.map((m: any, i: number) => (
                        <div key={i} className="flex gap-4 rounded-xl border border-gray-100 p-4">
                          {m.photo ? <img src={m.photo} alt={m.name} className="h-16 w-16 flex-shrink-0 rounded-full object-cover" /> : <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full bg-primary-100 text-primary-600">{listing.subcategory === 'skin-clinic' ? <DoctorIcon className="h-8 w-8" /> : <ScissorsIcon className="h-8 w-8" />}</div>}
                          <div className="min-w-0"><p className="font-semibold text-gray-800">{m.name}</p>{m.title && <p className="text-xs text-primary-600">{m.title}</p>}{m.bio && <p className="mt-1 text-xs leading-relaxed text-gray-500">{m.bio}</p>}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {menuItems.length > 0 && (
                  <div className="mt-8">
                    <SectionHeader icon={<MenuPriceIcon className="h-5 w-5" />} title={listing.subcategory === 'skin-clinic' ? td('treatmentsAndPrices') : listing.subcategory === 'hair-salon' ? td('servicesAndPrices') : listing.subcategory === 'sauna' ? td('addOnsAndCafeteria') : td('massageOptionsAndPrices')} />
                    <div className="divide-y divide-gray-100 rounded-xl border border-gray-100">
                ...
                {extra.important_notes?.length > 0 && <div className="mt-8"><SectionHeader icon={<WarningIcon className="h-5 w-5" />} title={td('thingsToKnow')} /><div className="grid gap-2 sm:grid-cols-2">{extra.important_notes.map((n: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg border border-amber-100 bg-amber-50 p-3"><span className="mt-0.5 flex-shrink-0 text-amber-500"><WarningIcon className="h-4 w-4" /></span><span className="text-sm text-gray-700">{n}</span></div>)}</div></div>}
              </>
            )}
            ...
            {cat === 'activities' && (
              <>
                ...
                {extra.age_pricing?.length > 0 && (
                  <div className="mt-8"><SectionHeader icon={<MoneyIcon className="h-5 w-5" />} title={td('pricingByAge')} /><div className="divide-y divide-gray-100 rounded-xl border border-gray-100">{extra.age_pricing.map((tier: { label: string; price: number }, i: number) => <div key={i} className="flex items-center justify-between px-4 py-3"><span className="font-medium text-gray-700">{tier.label}</span><span className="font-semibold text-primary-600">₩{Number(tier.price).toLocaleString()}</span></div>)}</div></div>
                )}
                ...
                {menuItems.length > 0 && (
                  <div className="mt-8"><SectionHeader icon={<ProgramIcon className="h-5 w-5" />} title={td('programOptions')} /><div className="divide-y divide-gray-100 rounded-xl border border-gray-100">{menuItems.map((item, i) => <div key={i} className="flex items-start justify-between px-4 py-3"><div><p className="font-medium text-gray-800">{item.name}</p>{item.description && <p className="text-xs text-gray-500">{item.description}</p>}</div>{item.price > 0 && <span className="ml-4 shrink-0 text-sm font-semibold text-primary-600">₩{item.price?.toLocaleString()}</span>}</div>)}</div></div>
                )}
                {(listing.address || listing.phone || extra.naver_map_url || extra.kakao_map_url || extra.google_map_url) && (
                  <div className="mt-8 rounded-xl border border-gray-100 p-5">
                    <div className="mb-3 flex items-center gap-2"><span className="text-primary-500"><PinIcon className="h-5 w-5" /></span><h2 className="text-lg font-bold text-gray-900">{td('contactAndLocation')}</h2></div>
                    <div className="grid gap-1 sm:grid-cols-2">
                      <InfoRow icon={<PinIcon />} label={td('address')} value={listing.address} />
                      <InfoRow icon={<PhoneIcon />} label={td('phone')} value={listing.phone} />
                    ...
                    <MapButtons />
                    {extra.google_map_url && <div className="mt-4 overflow-hidden rounded-lg"><iframe src={`https://www.google.com/maps?q=${encodeURIComponent(listing.address || extra.meeting_point || '')}&output=embed`} width="100%" height="250" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div>}
                  </div>
                )}
                {extra.reservation_notices?.length > 0 && <div className="mt-8"><SectionHeader icon={<InfoCircleIcon className="h-5 w-5" />} title={td('reservationInfo')} /><div className="space-y-2">{extra.reservation_notices.map((n: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg bg-blue-50 p-3"><span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-blue-200 text-xs font-bold text-blue-700">{i+1}</span><span className="text-sm text-gray-700">{n}</span></div>)}</div></div>}
                {extra.cancellation_policy?.length > 0 && <div className="mt-8"><SectionHeader icon={<RefreshIcon className="h-5 w-5" />} title={td('cancellationPolicy')} /><div className="space-y-2">{extra.cancellation_policy.map((p: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg bg-gray-50 p-3"><span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-gray-400" /><span className="text-sm text-gray-700">{p}</span></div>)}</div></div>}
      ...
      {/* Lightbox (inside modal, higher z-index) */}
      ...
      <div className="container-main py-8 max-w-3xl">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2">
          {listing.tags.map(tag => (
            <span key={tag} className={tag === 'HOT' ? 'badge-hot' : tag === 'BEST' ? 'badge-best' : tag === 'NEW' ? 'badge-new' : 'rounded-full bg-white border border-gray-200 px-3 py-1 text-xs font-medium text-gray-700'}>
              {tag}
            </span>
          ))}
          ...
          <span className="text-sm text-gray-500">{t(subcategory.labelKey)}</span>
        </div>
          ...
          <div className="mt-3">
            {bookingType === 'free' ? (
              <span className="rounded-full bg-green-100 px-4 py-1.5 text-sm font-semibold text-green-700">
                {t('booking.freeRequest')}
              </span>
            ) : (
              <div className="flex items-baseline gap-1.5">
                <span className="text-sm text-gray-500">{t('booking.fromLabel')}</span>
                <span className="text-2xl font-bold text-primary-600">₩{listing.price.toLocaleString()}</span>
        ...
        {/* Short description */}
        <p className="mt-4 leading-relaxed text-gray-600">{listing.description}</p>
        ...
        <div className="mt-8 flex gap-3">
          <button
            ...
            className="flex-1 rounded-xl border-2 border-gray-800 py-4 text-base font-bold text-gray-800 transition hover:bg-gray-50 active:scale-95"
          >
            {td('moreDetails')}
          </button>
          {!isTips && (
            <button
              onClick={openBooking}
              className="flex-1 rounded-xl bg-primary-500 py-4 text-base font-bold text-white shadow-md shadow-primary-200 transition hover:bg-primary-600 active:scale-95"
            >
              {td('selectDates')}
            </button>
          )}
        ...
        {showBooking && (
          <div ref={bookingRef} className="mt-10 scroll-mt-8">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900">{t('booking.title')}</h2>
              <button onClick={() => setShowBooking(false)} className="rounded-full bg-gray-100 px-3 py-1 text-sm text-gray-500 hover:bg-gray-200 transition">✕</button>
            </div>
