          ...
          <div ref={contentRef}>
              ...
              <div className="mt-4">
                {headerPrice > 0 ? (
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className="text-3xl font-bold text-primary-600">{formatPrice(headerPrice)}</span>
                            ...
                            {items.map((item, i) => (
                              <div key={i} className="flex items-start gap-3 px-4 py-3">
                                ...
                                <div className="flex flex-1 items-center justify-between">
                                  ...
                                  <span className="ml-4 shrink-0 text-sm font-semibold text-primary-600">{item.price_variable ? td('variablePrice') : formatPrice(item.price ?? 0)}</span>
                                </div>
                              </div>
                            ))}
                          </div>
            ...
            {cat === 'wellness' && (
              <>
                ...
                {listing.subcategory === 'sauna' && (extra.adult_price || extra.child_price) && <div className="mt-8"><SectionHeader icon={<MoneyIcon className="h-5 w-5" />} title={td('admissionPricing')} /><p className="mb-3 text-xs text-gray-400">{td('childAgeSauna')}</p><div className="grid gap-3 sm:grid-cols-2">{extra.adult_price > 0 && <div className="rounded-xl border border-gray-100 p-4 text-center"><p className="text-sm font-medium text-gray-500">{td('adultAgeLabel')}</p><p className="mt-1 text-2xl font-bold text-primary-600">{formatPrice(Number(extra.adult_price))}</p></div>}{extra.child_price > 0 && <div className="rounded-xl border border-gray-100 p-4 text-center"><p className="text-sm font-medium text-gray-500">{td('childAgeLabel')}</p><p className="mt-1 text-2xl font-bold text-primary-600">{formatPrice(Number(extra.child_price))}</p></div>}</div></div>}
