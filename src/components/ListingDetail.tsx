...
export default function ListingDetail({
  ...
  const INIT = 5;
  const extraCount = Math.max(0, allImages.length - INIT);
  const visibleImages = galleryExpanded ? allImages : allImages.slice(0, INIT);
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="container-main py-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[3fr_2fr]">
          ...
          <div>
            {/* Main image */}
            <div className="relative aspect-[4/3] cursor-pointer overflow-hidden rounded-2xl"
              onClick={() => setLbIdx(0)}>
              <Image src={allImages[0]} alt={listing.title} fill className="object-cover" priority sizes="(max-width:1024px) 100vw, 60vw" />
            ...
            {allImages.length > 1 && (
              <div className="mt-2 grid grid-cols-4 gap-2">
                {visibleImages.slice(1, 5).map((img, i) => {
                  const idx = i + 1;
                  const isLast = i === 3 && extraCount > 0 && !galleryExpanded;
                  return (
                    <div key={idx} className="relative aspect-[4/3] cursor-pointer overflow-hidden rounded-lg"
                      onClick={() => isLast ? setGalleryExpanded(true) : setLbIdx(idx)}>
                      <Image src={img} alt="" fill className="object-cover transition-transform duration-300 hover:scale-105" sizes="15vw" />
                      {isLast && (
            ...
            {galleryExpanded && extraCount > 0 && (
              <div className="mt-2 grid grid-cols-4 gap-2">
                {allImages.slice(5).map((img, i) => (
                  <div key={i+5} className="relative aspect-[4/3] cursor-pointer overflow-hidden rounded-lg" onClick={() => setLbIdx(i+5)}>
                    <Image src={img} alt="" fill className="object-cover transition-transform duration-300 hover:scale-105" sizes="15vw" />
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col">
            ...
            {/* Title */}
            <h1 className="mt-3 text-2xl font-bold leading-tight text-gray-900 lg:text-3xl">{listing.title}</h1>
            ...
            {/* Short description */}
            <p className="mt-4 text-sm leading-relaxed text-gray-600 line-clamp-4">{listing.description}</p>
            ...
            <div className="mt-auto pt-6 flex flex-col gap-3">
              <button
                ...
                className="w-full rounded-xl border-2 border-gray-800 py-3.5 text-base font-bold text-gray-800 transition hover:bg-gray-50 active:scale-95"
              >
                {td('moreDetails')}
              </button>
              {!isTips && (
                <button
                  ...
                  className="w-full rounded-xl bg-primary-500 py-3.5 text-base font-bold text-white shadow-md shadow-primary-200 transition hover:bg-primary-600 active:scale-95"
                >
                  {td('selectDates')}
                </button>
              )}
        ...
        {/* ══ DETAIL CONTENT (full width, below gallery) ══ */}
        <div ref={contentRef} className="mt-12 scroll-mt-8 border-t border-gray-100 pt-10">
                    ...
                    return (
                      <div key={c} className="mb-6">
                        <h3 className="mb-2 text-sm font-semibold uppercase tracking-wider text-gray-400">
                          {c === 'main' ? td('mainDishes') : c === 'side' ? td('sideDishes') : td('drinksAndAlcohol')}
                        </h3>
          ...
          {cat === 'wellness' && (
            <>
              ...
              {menuItems.length > 0 && <div className="mt-8"><SectionHeader icon={<MenuPriceIcon className="h-5 w-5" />} title={listing.subcategory === 'skin-clinic' ? td('treatmentsAndPrices') : listing.subcategory === 'hair-salon' ? td('servicesAndPrices') : listing.subcategory === 'sauna' ? td('addOnsAndCafeteria') : td('massageOptionsAndPrices')} /><div className="divide-y divide-gray-100 rounded-xl border border-gray-100">{menuItems.map((item, i) => <div key={i} className="flex items-start gap-3 px-4 py-3"><div className="flex flex-1 items-center justify-between"><div><p className="font-medium text-gray-800">{item.name}</p>{item.description && <p className="text-xs text-gray-500">{item.description}</p>}</div><span className="ml-4 shrink-0 text-sm font-semibold text-primary-600">₩{item.price?.toLocaleString()}</span></div></div>)}</div></div>}
              {extra.reservation_notices?.length > 0 && <div className="mt-8"><SectionHeader icon={<InfoCircleIcon className="h-5 w-5" />} title={td('reservationInfo')} /><div className="space-y-2">{extra.reservation_notices.map((n: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg bg-blue-50 p-3"><span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-blue-200 text-xs font-bold text-blue-700">{i+1}</span><span className="text-sm text-gray-700">{n}</span></div>)}</div></div>}
              {extra.cancellation_policy?.length > 0 && <div className="mt-8"><SectionHeader icon={<RefreshIcon className="h-5 w-5" />} title={td('cancellationPolicy')} /><div className="space-y-2">{extra.cancellation_policy.map((p: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg bg-gray-50 p-3"><span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-gray-400" /><span className="text-sm text-gray-700">{p}</span></div>)}</div></div>}
              {extra.important_notes?.length > 0 && <div className="mt-8"><SectionHeader icon={<WarningIcon className="h-5 w-5" />} title={td('thingsToKnow')} /><div className="grid gap-2 sm:grid-cols-2">{extra.important_notes.map((n: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg border border-amber-100 bg-amber-50 p-3"><span className="mt-0.5 flex-shrink-0 text-amber-500"><WarningIcon className="h-4 w-4" /></span><span className="text-sm text-gray-700">{n}</span></div>)}</div></div>}
            </>
          ...
          {cat === 'activities' && (
            <>
              ...
              {extra.age_pricing?.length > 0 && <div className="mt-8"><SectionHeader icon={<MoneyIcon className="h-5 w-5" />} title={td('pricingByAge')} /><div className="divide-y divide-gray-100 rounded-xl border border-gray-100">{extra.age_pricing.map((tier: { label: string; price: number }, i: number) => <div key={i} className="flex items-center justify-between px-4 py-3"><span className="font-medium text-gray-700">{tier.label}</span><span className="font-semibold text-primary-600">₩{Number(tier.price).toLocaleString()}</span></div>)}</div></div>}
              ...
              {(listing.address || listing.phone || extra.naver_map_url || extra.kakao_map_url || extra.google_map_url) && (
                <div className="mt-8 rounded-xl border border-gray-100 p-5">
                  ...
                  <MapButtons />
                  {extra.google_map_url && <div className="mt-4 overflow-hidden rounded-lg"><iframe src={`https://www.google.com/maps?q=${encodeURIComponent(listing.address || extra.meeting_point || '')}&output=embed`} width="100%" height="300" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" /></div>}
                </div>
              )}
              {extra.reservation_notices?.length > 0 && <div className="mt-8"><SectionHeader icon={<InfoCircleIcon className="h-5 w-5" />} title={td('reservationInfo')} /><div className="space-y-2">{extra.reservation_notices.map((n: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg bg-blue-50 p-3"><span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-blue-200 text-xs font-bold text-blue-700">{i+1}</span><span className="text-sm text-gray-700">{n}</span></div>)}</div></div>}
              {extra.cancellation_policy?.length > 0 && <div className="mt-8"><SectionHeader icon={<RefreshIcon className="h-5 w-5" />} title={td('cancellationPolicy')} /><div className="space-y-2">{extra.cancellation_policy.map((p: string, i: number) => <div key={i} className="flex items-start gap-3 rounded-lg bg-gray-50 p-3"><span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-gray-400" /><span className="text-sm text-gray-700">{p}</span></div>)}</div></div>}
        ...
        {/* ══ BOOKING SECTION (full width, at bottom) ══ */}
        {!isTips && (
          <div ref={bookingRef} className="mt-12 scroll-mt-8 border-t border-gray-100 pt-10">
