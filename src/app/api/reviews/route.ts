...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ reviews: reviews ?? [], canReview, myReview, bookingDate });
}
...
export async function POST(req: NextRequest) {
  ...
  const { listing_id, listing_title, rating, title, content } = body;
  ...
  const { data: profile } = await supabase
    ...
    .eq('id', user.id)
    .single();
  const reviewer_name = profile?.name || user.email?.split('@')[0] || 'Guest';
